...
def on_validate_excel(ctx: Ctx):
    ...
    try:
        ...
        if draft:
            ...
            ctx.save_job_state()
            ctx.app_state_save(ctx.state)
            ctx.log(f"✅ Migrated DRAFT job state to JSON for {env} @ {oper_key}.")

        show_dataframe_in_treeview(ctx.widgets["overview_tree"], df_final)

        if status == "duplicado":
            ctx.log(f"⚠️ Excel {env}: DUPLICATED snapshots (allowed)")
        elif status == "quase_duplicado":
            ctx.log(f"⚠️ Excel {env}: ALMOST duplicated snapshots (allowed)")
        else:
            ctx.log(f"✅ Excel {env}: snapshots OK")

        # se mudou operating date, recarrega jobs para evitar “rodada antiga na tela”
        if prev_oper_key and prev_oper_key != oper_key:
            ctx.log(f"ℹ️ Operating date changed: {prev_oper_key} -> {oper_key}. Reloading jobs view...")
...
def on_job_double_click(ctx: Ctx, event):
    ...
    if not oper_key:
        ...
        ctx.app_state_save(ctx.state)
        ctx.log(f"📝 Saved job state to DRAFT for {env} (persisted).")
        refresh_env_summary(ctx)
        update_button_states(ctx)
        return
...
def on_prepare_email(ctx: Ctx):
    ...
    # Jira automático só se:
    # - ainda não tem EVT no state
    # - tem df_last4
    # - tem operating date
    ...
    if (not jira_key and df_last4 is not None and ctx.state.operating_date_ddmmyyyy and ctx.state.bat_done):
        try:
            ...
            if not files:
                ctx.log("⚠️ No attachments found (CSV/Excel). Jira will be created without attachments.")
            else:
                ctx.log(f"📎 Attachments to send: {len(files)} file(s)")
            ...
            if issue_key:
                ...
                ctx.app_state_save(ctx.state)
                jira_key = issue_key
                ctx.log(f"✅ Jira auto-created: {issue_key}")
            else:
                ctx.log("ℹ️ Jira not created (world-ideal scenario).")
        ...
        ctx.log("⚠️ BAT not done yet — skipping Jira auto-create (sync not guaranteed).")
    ...
    ctx.log(f"📧 Email prepared ({email_type}) | Jira={jira_key}")
...
def on_retry_jira_upload(ctx: Ctx):
    ...
    try:
        ...
        if not files:
            ...
            ctx.log("⚠️ No attachment files found. Abort upload.")
            return

        ctx.log(f"📎 Found {len(files)} attachment files for env={env}.")
        ...
        if target_evt and not create_new:
            ...
            if not attached:
                ctx.log(f"ℹ️ Nothing new to upload to {target_evt} (all files already attached).")
                messagebox.showinfo("Jira Upload", f"No new files to upload.\n{target_evt} already has them.")
                return

            ctx.log(f"✅ Uploaded {len(attached)} new file(s) to {target_evt}")

            messagebox.showinfo("Success", f"Uploaded {len(attached)} new file(s) to {target_evt}")
            ctx.state.last_jira_key[env] = target_evt
            ctx.app_state_save(ctx.state)
            ctx.log(f"✅ Uploaded {len(attached)} file(s) to existing Jira: {target_evt}")
        ...
        if not issue_key:
            ctx.log("ℹ️ Jira not created (rule returned None).")
            messagebox.showinfo("Info", "Jira not created (rule logic prevented creation).")
            return

        ctx.state.last_jira_key[env] = issue_key
        ctx.app_state_save(ctx.state)
        ctx.log(f"✅ Jira created and files uploaded: {issue_key}")
