...
def try_create_jira(
    ...
    # só garante que o nome do grupo existe (QA/PROD)
...
def collect_jira_attachments(
        ...
        *,
        log=None,  # função opcional de log (ctx.log)
) -> List[Path]:
    """
    ...
    # ---- logging amigável ----
    if log:
        log(f"📎 Jira attachments: found {len(found_files)}/{len(legacy_jobs.get(env, []))} files")
        if missing:
            log(f"⚠️ Missing attachments: {', '.join(missing)}")
...
def list_attachment_names(issue_key: str) -> Set[str]:
    """
    Retorna o conjunto de nomes de anexos já presentes no issue.
    """
    jira = _make_jira_client()
...
def filter_new_files(issue_key: str, files: List[Path], *, log=None) -> List[Path]:
    """
    Remove da lista os arquivos cujo nome já existe no Jira.
    """
    ...
    if log:
        log(f"📎 Jira already has {len(existing)} attachment(s).")
        skipped = [p.name for p in files if p.name in existing]
        if skipped:
            log(f"⏭️ Skipping already-uploaded: {', '.join(skipped)}")
        log(f"⬆️ Will upload {len(new_files)}/{len(files)} new file(s).")
...
def attach_files_to_issue(issue_key: str, files: List[Path], *, skip_existing: bool = True, log=None) -> List[str]:
    """
    Anexa arquivos ao issue. Se skip_existing=True, não reenvia nomes já existentes.
    """
    to_upload = files
    if skip_existing:
